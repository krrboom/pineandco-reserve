...
const TR = {
  en: {
    ...
    rConfirmSms: r => `PINE&CO: Hi ${asciiName(r.name)}, your reservation is confirmed — ${r.date} ${r.time} for ${r.partySize}. Please arrive on time; 5+ min late is treated as a no-show and the table is released. Late or can't find the entrance? Call ${P()}`,
    rRem24Sms:   r => `PINE&CO: Hi ${asciiName(r.name)}, a friendly reminder — your reservation is on ${r.date} at ${r.time} for ${r.partySize}. Please arrive on time; 5+ min late is treated as a no-show. Late or can't find the entrance? Call ${P()}`,
    rRem1hSms:   r => `PINE&CO: Hi ${asciiName(r.name)}, your reservation is coming up at ${r.time} today for ${r.partySize}. See you soon! 5+ min late is treated as a no-show. Late or can't find the entrance? Call ${P()}`,
    rTitle: 'YOUR RESERVATION',
    rLabels: { date:'Date', time:'Time', name:'Name', party:'Party', seat:'Seat' },
    rPolicy: `<p style="margin:0 0 8px;">Please arrive on time. As a courtesy to all our guests, tables more than <strong style="color:#c9a96e;">5 minutes late</strong> are treated as a no-show and released.</p><p style="margin:0 0 8px;">Running a little late, or having trouble finding the entrance? Please give us a call — we'd be glad to help you in.</p><p style="margin:0;">To change or cancel your reservation, please call us at <strong style="color:#b8935a;">${CONFIG.BUSINESS_PHONE}</strong> — changes and cancellations are by phone only.</p>`,
    ...
    cSms:  (name, kindW, url) => `PINE&CO: ${asciiName(name)}, we waited 5 minutes but didn't hear from you, so your ${kindW} has been cancelled. We're sorry to miss you — we'd love to see you next time. Reserve: ${url}`,
    cHeading: kind => kind === '예약' ? 'RESERVATION RELEASED' : 'WAITLIST RELEASED',
    cGreeting: name => `See you next time,<br>${name}.`,
    cBody: kindW => `We waited 5 minutes but didn't hear from you, so your ${kindW} has been cancelled. We're truly sorry to miss you, and we'd love to welcome you next time.`,
  ...
  ko: {
    ...
    rConfirmSms: r => `[PINE&CO] ${r.name}님, 예약이 확정되었습니다 — ${r.date} ${r.time}, ${r.partySize}명. 정시 도착 부탁드려요. 5분 이상 늦으면 노쇼 처리됩니다. 늦거나 입구를 못 찾으시면 전화 주세요: ${P()}`,
    rRem24Sms:   r => `[PINE&CO] ${r.name}님, 예약 안내 — ${r.date} ${r.time}, ${r.partySize}명. 정시 도착 부탁드려요. 5분 이상 늦으면 노쇼 처리됩니다. 늦거나 입구 못 찾으시면 전화: ${P()}`,
    rRem1hSms:   r => `[PINE&CO] ${r.name}님, 곧 예약 시간이에요 — 오늘 ${r.time}, ${r.partySize}명. 5분 이상 늦으면 노쇼 처리됩니다. 늦거나 입구 못 찾으시면 전화: ${P()}`,
    rTitle: '예약 안내',
    rLabels: { date:'날짜', time:'시간', name:'이름', party:'인원', seat:'좌석' },
    rPolicy: `<p style="margin:0 0 8px;">정시 도착 부탁드립니다. 다른 손님을 위해 예약 시간보다 <strong style="color:#c9a96e;">5분 이상</strong> 늦으시면 노쇼로 처리됩니다.</p><p style="margin:0 0 8px;">조금 늦으시거나 입구를 못 찾으시면 전화 주세요 — 기꺼이 안내해 드릴게요.</p><p style="margin:0;">예약 변경 및 취소는 전화(<strong style="color:#b8935a;">${CONFIG.BUSINESS_PHONE}</strong>)로만 가능합니다.</p>`,
    zone: { bar:'바 좌석', table:'테이블', highTable:'하이테이블', room:'프라이빗 룸' },
    cSubj: `Pine & Co — 다음에 또 뵐게요`,
    cKind: kind => kind === '예약' ? '예약' : '웨이팅',
    cSms:  (name, kindW, url) => `[PINE&CO] ${name}님, 5분을 기다렸으나 연락이 없어 ${kindW}이(가) 취소되었습니다. 다음에 꼭 뵙길 바랍니다. 예약: ${url}`,
    cHeading: kind => kind === '예약' ? '예약이 취소되었습니다' : '웨이팅이 취소되었습니다',
    cGreeting: name => `${name}님,<br>다음에 또 뵐게요.`,
    cBody: kindW => `5분을 기다렸으나 연락이 없어 ${kindW}이(가) 취소되었습니다. 진심으로 아쉬우며, 다음엔 꼭 편하게 모시고 싶어요.`,
    cBtn: '예약하기',
    cClose: `다음에 꼭 뵙겠습니다.`,
  ...
  zh: {
    ...
    rConfirmSms: r => `[PINE&CO] ${r.name}，您的预订已确认 — ${r.date} ${r.time}，${r.partySize}位。请准时到达；迟到超过5分钟将视为爽约。迟到或找不到入口请致电：${P()}`,
    rRem24Sms:   r => `[PINE&CO] ${r.name}，预订提醒 — ${r.date} ${r.time}，${r.partySize}位。请准时到达；迟到超过5分钟将视为爽约。迟到或找不到入口请致电：${P()}`,
    rRem1hSms:   r => `[PINE&CO] ${r.name}，您的预订即将开始 — 今天 ${r.time}，${r.partySize}位。迟到超过5分钟将视为爽约。迟到或找不到入口请致电：${P()}`,
    rTitle: '您的预订',
    rLabels: { date:'日期', time:'时间', name:'姓名', party:'人数', seat:'座位' },
    rPolicy: `<p style="margin:0 0 8px;">请准时到达。为照顾所有客人，迟到超过 <strong style="color:#c9a96e;">5 分钟</strong> 的座位将被视为爽约并释放。</p><p style="margin:0 0 8px;">稍有迟到，或找不到入口？请致电，我们乐意为您引导入内。</p><p style="margin:0;">预订的更改或取消仅可致电 <strong style="color:#b8935a;">${CONFIG.BUSINESS_PHONE}</strong> 办理。</p>`,
    zone: { bar:'吧台', table:'餐桌', highTable:'高桌', room:'包间' },
    cSubj: `Pine & Co — 期待下次光临`,
    cKind: kind => kind === '예약' ? '预订' : '候位',
    cSms:  (name, kindW, url) => `[PINE&CO] ${name}，我们等候了5分钟但未收到您的联系，您的${kindW}已取消。期待下次光临。预订：${url}`,
    cHeading: kind => kind === '예약' ? '预订已取消' : '候位已取消',
    cGreeting: name => `${name}，<br>期待下次光临。`,
    cBody: kindW => `我们等候了5分钟但未收到您的联系，您的${kindW}已取消。非常抱歉，期待下次能为您服务。`,
    cBtn: '立即预订',
    cClose: `期待与您再次相见。`,
  ...
  ja: {
    ...
    rConfirmSms: r => `[PINE&CO] ${r.name}様、ご予約が確定しました — ${r.date} ${r.time}、${r.partySize}名様。時間どおりのご来店をお願いします。5分以上遅れるとキャンセル扱いとなります。遅れる・入口が分からない場合はお電話ください：${P()}`,
    rRem24Sms:   r => `[PINE&CO] ${r.name}様、ご予約のリマインダー — ${r.date} ${r.time}、${r.partySize}名様。時間どおりのご来店を。5分以上遅れるとキャンセル扱いとなります。遅れる・入口が分からない時はお電話：${P()}`,
    rRem1hSms:   r => `[PINE&CO] ${r.name}様、まもなくご予約のお時間です — 本日 ${r.time}、${r.partySize}名様。5分以上遅れるとキャンセル扱いとなります。遅れる・入口が分からない時はお電話：${P()}`,
    rTitle: 'ご予約',
    rLabels: { date:'日付', time:'時間', name:'お名前', party:'人数', seat:'お席' },
    rPolicy: `<p style="margin:0 0 8px;">時間どおりのご来店をお願いします。他のお客様への配慮のため、ご予約時間より <strong style="color:#c9a96e;">5分以上</strong> 遅れるとキャンセル扱いとなります。</p><p style="margin:0 0 8px;">少し遅れる、または入口が分からない場合はお電話ください。喜んでご案内します。</p><p style="margin:0;">ご予約の変更・キャンセルはお電話（<strong style="color:#b8935a;">${CONFIG.BUSINESS_PHONE}</strong>）のみで承ります。</p>`,
    zone: { bar:'バー', table:'テーブル', highTable:'ハイテーブル', room:'個室' },
    cSubj: `Pine & Co — またのお越しを`,
    cKind: kind => kind === '예약' ? 'ご予約' : 'ウェイティング',
    cSms:  (name, kindW, url) => `[PINE&CO] ${name}様、5分お待ちしましたがご連絡がなく、${kindW}はキャンセルとなりました。次回のご来店をお待ちしています。ご予約：${url}`,
    cHeading: kind => kind === '예약' ? 'ご予約はキャンセルされました' : 'ウェイティングはキャンセルされました',
    cGreeting: name => `${name}様、<br>またのお越しをお待ちしています。`,
    cBody: kindW => `5分お待ちしましたが、ご連絡がなく${kindW}はキャンセルとなりました。誠に申し訳ございません。次回ぜひお待ちしております。`,
    cBtn: '予約する',
